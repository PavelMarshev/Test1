...
def test_get_today_cash_remained_two():
    cash_calculator = CashCalculator(1000) # init calc
    records = [Record(amount=1000, comment='кофе'),
               Record(amount=3000, comment='Серёге за обед', date='08.11.2019'),
               Record(amount=3000, comment='бар в Танин др', date='08.11.2019')] # create records
    for record in records:
        cash_calculator.add_record(record) # insert records
    amounts = records[0].amount
    assert cash_calculator.get_today_cash_remained(" usd") == "Ты на мели, бро" + str(-1*(1000 - amounts)) + " usd"


def test_get_calories_remained_two():
    calories_calculator = CaloriesCalculator(1000) # init calc
    records = [Record(amount=145, comment='кофе', date='08.11.2019'),
               Record(amount=300, comment='Серёге за обед', date='08.11.2019'),
               Record(amount=3000, comment='бар в Танин др', date='08.11.2019')] # create records
    for record in records:
        calories_calculator.add_record(record) # insert records
    assert calories_calculator.get_calories_remained() == "Свободных средств:" + str(1000)
